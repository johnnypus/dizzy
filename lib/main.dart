...
/// Flag defensivo para evitar reentradas en el mismo isolate.
bool _firebaseInitStarted = false;

/// Inicializa Firebase sin duplicar la app DEFAULT.
/// - Si ya existe, solo devuelve.
/// - Si intenta duplicar, ignora el error 'duplicate-app'.
Future<void> _ensureFirebase() async {
  if (_firebaseInitStarted) return;
  _firebaseInitStarted = true;

  // Si ya hay una app (DEFAULT) disponible, no hagas nada.
  try {
    Firebase.app();
    return;
  } catch (_) {
    // No existe aún, seguimos a inicializar.
  }
  ...
  try {
    if (GetPlatform.isWeb) {
      ...
      // iOS/macOS usa GoogleService-Info.plist
      await Firebase.initializeApp();
    }
  } on FirebaseException catch (e) {
    // Si otro punto inicializó en carrera, ignora el duplicado
...
Future<void> main() async {
  ...
  // --- Firebase: init robusta sin duplicados ---
  await _ensureFirebase();

  // Registrar handler de background (si ya estaba, no pasa nada)
  try {
